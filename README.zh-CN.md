...
**spring-ai-starter-model-hermes** 是一个面向 使用 spring-ai-model-hermes 的应用 的 Spring Boot Starter，用于将 **spring-ai-model-hermes** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 spring-ai-model-hermes 的全部能力。
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 spring-ai-model-hermes 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `spring.ai.model.hermes.*` 到 `OpenClawSpringAiProperties` |
| `OpenClawApi` Bean | ✅ 稳定 | 通过 OpenClawSpringAiAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `OpenClawApi` | classpath + property | 不创建 |
| `OpenClawResponsesApi` | classpath + property | 不创建 |
| `OpenClawChatOptions` | classpath + property | 不创建 |
| `ChatModel` | classpath + property | 不创建 |
| `EmbeddingModel` | classpath + property | 不创建 |
| `OpenClawResponsesModel` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `spring.ai.model.hermes.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `spring.ai.model.hermes.enabled=true` 与 classpath |
